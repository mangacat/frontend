    ...
    <div bind:this={quick_search_container} transition:fly="{{ y: -20, duration: 250 }}" class="rounded overflow-hidden absolute shadow" style="top: 5rem; width: 600px;">
        ...
        <div class="results-list">
            ...
            {:then results}
                ...
                {/each}
                <a href="/search{search_value !== '' ? `?name=${search_value}` : ''}" class="bg-gray-100 dark:bg-gray-700 flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-800 text-sm py-2" on:click={close}>
                    Advanced Search...
                </a>
            {/await}
        </div>
...
<script>
    ...
    onMount(() => { quick_search_input.focus() })
</script>