<div class="absolute inset-0 h-screen flex justify-center z-50">
    ...
    <div transition:fly="{{ y: -20, duration: 250 }}" class="rounded overflow-hidden absolute shadow" style="top: 5rem; width: 600px;">
        <div class="relative">
            <input on:keydown={enter} bind:value={search_value} bind:this={quick_search_input} class="bg-gray-100 dark:bg-gray-700 outline-none py-3 pr-4 pl-10 block appearance-none leading-normal w-full" type="search" placeholder="Search" />
        ...
        <div class="results-list">
            ...
            {:then results}
				{#if results.data}
                {#each results.data.series as {id, name, cover}}
...
import client, { SIMPLE_SEARCH } from "utils/apollo.js"
...
const search =  query(client, {query: SIMPLE_SEARCH, variables: {
	limit: 0,
	offset: 0,
	name: "",
}})
...
$: search.refetch({
	limit: 5,
	offset: 0,
	name: search_value!== ""? "%" + search_value + "%": null,
})
