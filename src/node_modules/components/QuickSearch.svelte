<div class="absolute inset-0 h-screen flex justify-center z-50">
    ...
    <div transition:fly="{{ y: -20, duration: 250 }}" class="rounded overflow-hidden absolute shadow" style="top: 5rem; width: 600px;">
        <div class="relative">
            <input bind:value={search_value} bind:this={quick_search_input} class="bg-gray-100 dark:bg-gray-700 outline-none py-3 pr-4 pl-10 block appearance-none leading-normal w-full" type="search" placeholder="{$_.capital('search')}" />
        ...
        <div class="results-list">
            ...
            {:then results}
                {#each results as {id, name, cover}}
                    <a transition:slide href="/series/{id}/{slugify(name)}" class="bg-gray-100 dark:bg-gray-700 flex items-center relative px-4 pt-2 first-child:pt-3 pb-2 hover:bg-gray-200 dark:hover:bg-gray-800 h-16 relative" on:click={close}>
                ...
                {/each}
                <a href="/search{search_value !== '' ? `?name=${search_value}` : ''}" class="bg-gray-100 dark:bg-gray-700 flex items-center justify-center hover:bg-gray-200 dark:hover:bg-gray-800 text-sm py-2 capitalize" on:click={close}>
                    {$_('advanced_search')}
                </a>
            {/await}
        </div>
