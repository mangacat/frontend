            ...
            <ul class="flex items-center tracking-tight capitalize">
                {#if $session.user}
                    <li class="text-gray-900 dark:text-gray-100 mr-6 relative">
                        <div bind:this={trigger} class="cursor-pointer {$session.user.avatar_url ? 'h-10 w-10 rounded bg-cover bg-center' : ''}" style="{$session.user.avatar_url ? `background-image: url('${cdn($session.user.avatar_url, { resize: '80,80' })}');` : ''}">
                        ...
                        {#if dropdown_open}
                            <div transition:slide="{{ duration: 200 }}" class="select-none rounded absolute right-0 bg-white dark:bg-gray-700 shadow mt-1 py-2 border border-gray-300 dark:border-gray-900">
                                <a class="block w-full py-2 px-4 flex items-center hover:bg-gray-200 dark:hover:bg-gray-800" href="/user/{$session.user.id}/{slugify($session.user.display_name)}">
                                ...
                                <a class="block w-full py-2 px-4 flex items-center hover:bg-gray-200 dark:hover:bg-gray-800" href="/follows">
                                    <svg class="h-5 w-5 fill-current mr-3" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">
                ...
                <li class="text-gray-900 hover:text-gray-600 dark:text-gray-100 dark:hover:text-gray-400">
                    <svg class="w-6 h-6 cursor-pointer fill-current" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" on:click={() => { quick_search = true }}>
...
{#if quick_search}
    <QuickSearch on:close={() => { quick_search = false }} />
{/if}
...
<script>
    ...
    import Icon from 'components/Logos/Icon.svelte'
