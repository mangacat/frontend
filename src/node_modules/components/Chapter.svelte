    ...
    <div class="flex-grow">
        ...
        <div class="text-sm">
            <span title="{new Date(time_uploaded)}">
                ...
                •
            {#each groups_series_chapters as groups, i}
                {#if i != 0}
                        |
                {/if}
                <a class="hover:underline" href="/group/{groups.groups_scanlation_series_chapters_groups.id}/{slugify(groups.groups_scanlation_series_chapters_groups.name)}">{groups.groups_scanlation_series_chapters_groups.name}</a>
            {/each}
        </div>
    </div>
...
<script>
    ...
    const { number_volume, number_absolute, time_uploaded, groups_series_chapters, views, comments, title, id } = chapter
</script>
