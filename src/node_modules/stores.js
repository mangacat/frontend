...
const session = () => {
	...
	return {
		...
		async login(credentials) {
			const response = await post(`http://auth.manga.cat/auth/local/login`, credentials)
			console.log(response)
			const user = await fetch_get(`http://auth.manga.cat/auth/user`, {
		...
		async register(user) {
			const response = await post(`http://auth.manga.cat/auth/local/register`, user)
			return response
		},
