				...
				<div>
					...
					<div class="mt-2 {activeLink === '' ? 'flex flex-wrap' : ''}">
						...
						{:else if activeLink === 'crew'}
							{#if series.people_series.filter(x => x.type !== "" ? x.type.toLowerCase().includes('story'): "Unknown").length !== 0 }
								<div class="flex leading-none pt-2 max-w-md">
									...
									<div class="w-1/2 px-2 flex flex-wrap">
										{#each series.people_series.filter(x =>  x.type !== "" ? x.type.toLowerCase().includes('story'): "Unknown") as { people, type }}
							...
							{#if series.people_series.filter(x => x.type.toLowerCase().includes('art')).length !== 0}
								<div class="flex leading-none pt-2 max-w-md">
									...
									<div class="w-1/2 px-2 flex flex-wrap">
										{#each series.people_series.filter(x => x.type.toLowerCase().includes('art')) as { people, type }}
											<a href="/people/{people.id}/{slugify(people.name)}" class="cursor-pointer py-1 px-2 bg-gray-400 hover:bg-gray-500 dark:bg-gray-700 dark:hover:bg-gray-600 rounded mr-2 mb-2 text-sm">
												{people.name}
											</a>
										{/each}
