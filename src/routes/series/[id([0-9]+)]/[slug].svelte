<script context="module">
    ...
    import client, {SERIES} from "utils/apollo.js"
...
<script>
    ...
    export let cache
    restore(client, SERIES, cache.data)
    const series = query(client, {
    	query: SERIES,
    }
)
...
<div class="series-wrapper min-h-screen px-4 pt-8 max-w-5xl mx-auto mb-8">
    ...
    {:then result}
        {#if result.data}
            {#each result.data.series as series, index}
                <div>
                    ...
                    <div id="cover-container" class="sticky" style="top: { $nav_height + 25}px;">
                        ...
                        {#if series.raw || series.amzjp_id}
                            <div class="border border-gray-500 dark:border-gray-600 rounded overflow-hidden mt-2 series-official-links">
                                ...
                                {/if}
                                {#if series.amazonjp_id}
                                    <p class="py-2 mx-2 text-sm">
                                        <a href="https://www.amazon.co.jp/dp/{series.amazonjp_id}" rel="noreferrer noopener nofollow" class="inline-block align-middle cursor-pointer hover:text-gray-600 dark:hover:text-gray-400" target="_blank">
                ...
                <div>
                    ...
                    <div class="mt-2 {activeLink === '' ? 'flex flex-wrap' : ''}">
                        {#if activeLink === ''}
                            {#each series.tags_series as tags}
                                <a href="/search?tags_inc={tags.tags_series.tag_name}" class="cursor-pointer py-1 px-2 bg-gray-400 hover:bg-gray-500 dark:bg-gray-700 dark:hover:bg-gray-600 rounded mr-2 mt-2 text-sm">
                                    {tags.tags_series.tag_name}
                                </a>
                            {/each}
                        {:else if activeLink === 'crew'}
                            {#if series.people_series.filter(x => x.type.toLowerCase().includes('story')).length !== 0 }
                                <div class="flex leading-none pt-2 max-w-md">
                                    ...
                                    <div class="w-1/2 px-2 flex flex-wrap">
                                        {#each series.people_series.filter(x => x.type.toLowerCase().includes('story')) as people}
                                            <a href="/people/{people.people_series.id}/{slugify(people.people_series.name)}" class="cursor-pointer py-1 px-2 bg-gray-400 hover:bg-gray-500 dark:bg-gray-700 dark:hover:bg-gray-600 rounded mr-2 mb-2 text-sm">
                                                {people.people_series.name}
                                            </a>
                                        {/each}
                                    </div>
                                </div>
                            {/if}
                            {#if series.people_series.filter(x => x.type.toLowerCase().includes('art')).length !== 0}
                                <div class="flex leading-none pt-2 max-w-md">
                                    <div class="flex-grow">
                                        <h3 class="uppercase tracking-wide text-sm border-b border-dotted border-gray-700 dark:border-gray-100">
                                            <span class="bg-gray-100 dark:bg-gray-800 relative pr-1" style="top: 2px;">
                                    ...
                                    <div class="w-1/2 px-2 flex flex-wrap">
                                        {#each series.people_series.filter(x => x.type.toLowerCase().includes('art')) as people}
                                            <a href="/people/{people.people_series.id}/{slugify(people.people_series.name)}" class="cursor-pointer py-1 px-2 bg-gray-400 hover:bg-gray-500 dark:bg-gray-700 dark:hover:bg-gray-600 rounded mr-2 mb-2 text-sm">
                                                {people.people_series.name}
                                            </a>
                                        {/each}
                        ...
                        {:else}
                            ...
                            <div class="flex leading-none py-2 max-w-md">
                                ...
                                <div class="w-1/2 px-2 text-sm leading-snug">
                                    {series.series_aliases.map(x => x.name).join(', ')}
                                </div>
                            </div>
                        {/if}
                    </div>
                    ...
                    {/if}
                    {#if series.series_related.length !== 0}
                    ...
                    {/if}
                    {#if series.series_chapters.length !== 0}
                        <div class="mt-12 relative">
                            ...
                            {#each series.series_chapters as chapter}
                                <Chapter {chapter} />
                            {/each}
                        </div>
                    {/if}
