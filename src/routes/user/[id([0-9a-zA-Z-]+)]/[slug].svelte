...
<script context="module">
  import client,  { USER } from "utils/apollo.js"
...
restore(client, USER, cache.data)

const user = query(client, { query: USER })
</script>
...
<div class="min-h-screen max-w-9xl mx-auto px-4 mt-8">
  ...
  {:then result}
    {#if result.data}
      {#each result.data.users as user, index}
			<div class="rounded-lg w-full h-64 bg-cover bg-center relative flex items-end shadow" alt="{user.display_name}" style="background-image: url('{cdn(user.cover_image, { h: 512 })}');">
				<div class="h-full w-full flex flex-col justify-end">
					<div class="flex ml-4 md:ml-8">
						<img class="rounded-t md:rounded w-32 h-32 md:-mb-8 z-20 object-cover object-center md:shadow-md bg-gray-300 dark:bg-gray-700" alt="{user.display_name}" src="{cdn(user.avatar_url, { resize: '256,256' }) || '/img/mugshot.png'}" />
						<div class="ml-4 text-gray-300">
							<div class="mt-6">
								<span class="leading-none py-2 px-2 font-extrabold text-xl" style="background-color: rgba(26, 32, 44, 0.6); text-shadow: 0 1px 2px rgba(0, 0, 0, 0.2);">{user.display_name}</span>
							</div>
							<div class="mt-4">
								<span class="leading-none py-1 px-2" style="background-color: rgba(26, 32, 44, 0.6); text-shadow: 0 1px 2px rgba(0, 0, 0, 0.2);">{user.display_name}</span>
							</div>
						</div>
					</div>
					<div class="md:pl-40 py-2 rounded-b-lg bg-gray-300 dark:bg-gray-700 flex text-sm font-bold tracking-wide">
						<div class="flex-grow flex items-center">
							<span class="ml-4 py-1 border-r border-gray-500 dark:border-gray-600 mr-2 pr-2">Joined {timeAgo(user.created_at)} ago</span>
							<span class="py-1">Last seen {timeAgo(user.updated_at)} ago</span>
						</div>
